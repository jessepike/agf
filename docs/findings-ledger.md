...
**Last updated:** 2026-05-01 (3-model multi-lens pressure-test ingested — Gemini 3.1 Pro / GPT-5 Codex / Opus 4.7; 18 new atomic findings + 1 cross-model meta filed; 3 resolved same-session via factual fixes to GDR doc + agentic-primitives.md vocabulary)
**Open findings:** 16 (MI-F07 + 15 from 2026-05-01 multi-lens — see new rows below)
**Validated:** 17 (unchanged from 2026-04-24)
**Resolved (pending validation):** 23 (was 20; +GEM-F01 EU AI Act Art. 12 mapping correction in GDR doc; +GEM-F02 ISO 42001 Clause 9.3 mapping correction in GDR doc; +O47-F01 Multi-Agent Topology Patterns vocabulary rename in agentic-primitives.md)
**Accepted (continuous quality dimension):** 1 (G5-M03 — F33 adoption-proof gap remains structural; first case study required for closure)
**Accepted / queued:** 1 (G5-F14 infra shipped — content batch-apply still gated on primitive stability decision)
**Deferred:** 4 (MI-F03 collapsed; MI-F06 → tooling sprint; G5-F18 → editorial pass; G5-F33 → adoption case study Roadmap)
**Triage progress:** Cluster A Mechanical ✓ | Cluster B Credibility (partial) ✓ | Cluster C Differentiation ✓ | Cluster E Clarity (quick wins) ✓ | Cluster F Standards Fidelity (NEW 2026-05-01) ◔
**Reviews ingested:** 5

- `docs/reviews/2026-04-23-gpt-5-4.md` (external model — GPT-5.4)
- `docs/reviews/2026-04-23-mechanical-baseline.md` (internal tooling)
- `docs/reviews/2026-05-01-gemini-3-1-pro.md` (external model — Gemini 3.1 Pro; lens: senior standards analyst)
- `docs/reviews/2026-05-01-gpt-5-5.md` (external model — GPT-5 Codex; lens: skeptical principal engineer, regulated FS)
- `docs/reviews/2026-05-01-opus-4-7.md` (external model — Claude Opus 4.7; lens: systems architect, deep integrative read)
**Validation passes:** 1
...
| G5-F32 | Transition-state leakage on flagship pages weakens "ready now" impression — `what-is-agf.mdx:105` audit-in-flight caveat, `ai-engineering.mdx:128` queued-diagram-regen note, `roadmap.mdx:14` "in flight" framing | Low | Established | Public Readiness | **resolved** 2026-04-25 (Path A public-readiness pass shipped commit `cdea367`: confidence-marker audit completed across 4 previously-unmarked flagship pages — `agentic-observability`, `governance-decision-record` Standards Alignment, `trust-ladders`, `decision-intelligence`; `composition-patterns.png` regenerated as 4-pattern progression replacing retired `implementation-phases-roadmap.png`; both transition-state caveats removed since the underlying work shipped) |
...
| GEM-F01 | False equivalency between GDR (static decision artifact) and EU AI Act Art. 12 (continuous automatic event recording) | High | Established | Defensibility | **resolved** 2026-05-01 (GDR doc § Standards alignment rewritten — EU AI Act Art. 12 mapping primary path is now Agentic Observability events, not GDRs; GDR mapping clarified to Art. 14 Human Oversight as primary; cross-link to observability-profile.md added) |
| GEM-F02 | Mischaracterization of ISO/IEC 42001 — GDR mapped to Clause 9.3 (Management Review) is a severe stretch | Critical | Established | Credibility | **resolved** 2026-05-01 (GDR doc § Standards alignment — Clause 9.3 mapping removed; replaced with Clause 8.1 Operational planning + retained Clause 7.5 Documented information; explicit `<Confidence level="informed">` block added flagging that Annex A controls are not yet independently verified — queued for pre-v0.3 standards-fidelity audit) |
| GEM-F03 | Taxonomical inversion of seven-layer landscape stack — calling AGF "Layer 0" while NIST/ISO/EU AI Act are above is read by GRC reviewers as architectural hubris | Medium | Informed | Differentiation | **open** 2026-05-01 — sacred-file (DECISIONS.md #4); proposed clarification (clarify "Layer 0" is concrete-architectural-substrate, not authority-hierarchy; standards remain authoritative regardless of stack position) drafted at `.private/drafts/seven-layer-stack-clarification-2026-05-01.md`; Jesse decision required |
| GEM-F04 | Lapses into subjective marketing voice ("North Star", "playbook") in canonical reference docs vs RFC 2119 normative voice expected by GRC reviewers | Low | Open | Public Readiness | **open** 2026-05-01 — sacred-file (intent.md voice); position-paper response drafted at `.private/drafts/intent-voice-position-2026-05-01.md` (recommend: keep humility voice in intent.md per pillar #1 "Humility before authority"; harden RFC-2119 voice in canonical reference docs only); Jesse decision required |
| G55-F01 | GDR/event dual emission has no atomicity contract — partial-failure modes (event emitted, GDR write failed; duplicate retries; decision_id collision under burst load) are unspecified | High | Established | Actionability | **open** 2026-05-01 — structural; requires Runtime Integration Contract (new doc); queued in BACKLOG as new structural blocker class |
| G55-F02 | ESCALATE and DEFER are modeled as `resolved` even when operationally unresolved — system reports closed governance decisions while no authorized human has approved/rejected/modified | High | Established | Coherence | **open** 2026-05-01 — structural; requires GDR lifecycle revision (introduce `deferred_pending` / `escalated_pending` non-terminal states with `owner`/`due_at`/`fallback_default_action`); queued in BACKLOG |
| G55-F03 | Concurrent graph-mode lacks signal collision/precedence rules — single-return contract fits wrapper mode, not graph-embedded mode where Ring 1 PASS + Ring 2 GATE + Security HALT can race | High | Established | Coherence | **open** 2026-05-01 — structural; requires signal arbitration table (HALT dominates; ERROR(halt) → HALT; GATE blocks release unless superseded; REVISE invalidates prior PASS; PASS only final after bounded quiet period or quorum); queued in BACKLOG |
| G55-F04 | DELEGATE treated as internal control flow when it is often governed authority transfer (cross-agent task scope, data access, audit responsibility move together) | Medium | Established | Actionability | **open** 2026-05-01 — structural; requires DELEGATE handshake spec (DELEGATE_REQUEST / DELEGATE_ACCEPTED / DELEGATE_REJECTED / DELEGATE_EXPIRED); queued in BACKLOG |
| G55-F05 | Confidence-system page reintroduces overclaim ("AGF labels every substantive claim") softer-version had already replaced elsewhere | Medium | Established | Credibility | **open** 2026-05-01 — editorial fix (change to "load-bearing empirical, comparative, and novel claims" + add coverage note + audit checklist); pair with G5-F18 editorial pass |
| G55-F06 | "Deletion test" not answered where positioning burden lands — replacement-cost matrix missing (what would actually be lost without AGF, vs source frameworks providing it directly/partially/not at all) | Medium | Established | Differentiation | **open** 2026-05-01 — content (new "What you lose without AGF" matrix on relationship-to-frameworks.md); CMO/CPO scope; queued in BACKLOG |
| G55-F07 | F33 (adopter case study) is NOT the only structural blocker — runtime integration contract is co-equal structural blocker for regulated adoption | High | Established | Public Readiness | **open** 2026-05-01 — meta-finding; promotes new structural blocker class (Runtime Contract Hardening) into Roadmap; reframes M03 closure path: requires both adoption proof AND runtime contract |
| O47-F01 | "Composition Patterns" overloaded — same heading used in shared-vocabulary.md (4 canonical governance patterns per D10a) and agentic-primitives.md Multi-Agent Coordination section (3 topology patterns) | High | Established | Coherence | **resolved** 2026-05-01 (agentic-primitives.md:1289 renamed to "Multi-Agent Topology Patterns" with vocabulary-note callout cross-referencing D10a; shared-vocabulary disambiguation row pending sync) |
| O47-F02 | REVISE(context) → invalidated-approval GDR has no matching lifecycle transition — per-signal table promises GDR emission for stale-approval invalidation; lifecycle table only admits clock-driven `expired` (not event-driven invalidation) | High | Established | Mechanical Integrity | **open** 2026-05-01 — structural; three architectural options listed (widen `expired` semantics; require successor GDR with `superseded`; add new `invalidated` state); requires CPO/Jesse pick; queued in BACKLOG |
| O47-F03 | DELEGATE / ERROR(degrade) GDR emission is non-local to the issuer — depends on receiving Ring 2's policy, breaking OTAA local-auditability invariant | Medium | Established | Coherence | **open** 2026-05-01 — structural; requires issuer-side `delegation_attempted`/`degradation_proposed` observability event with `gdr_required: true|false|conditional` field; pair with G55-F04 DELEGATE handshake work |
| O47-F04 | No specified intersection between Composition Patterns × Maturity Levels × Risk-Tier × Modes — primer claims orthogonality but no cross-axis matrix exists; assessor cannot conformance-check | Medium | Established | Actionability | **open** 2026-05-01 — content; new normative Pattern × Maturity × Risk-Tier matrix in agentic-governance-framework.md required; identified by Opus as highest-leverage editorial fix (single change moves Actionability + Coherence simultaneously) |
| O47-F05 | Multi-agent governance recursion has only depth cap (`max 3 levels`) where environment-recursion has substantive trust anchor (signed root config); same recursion problem, asymmetric termination | Medium | Established | Coherence | **open** 2026-05-01 — structural; recommend extending environment-recursion trust-anchor pattern to multi-agent (outermost Ring 2 policy = trust anchor; human-authored; signed; reviewed); queued in BACKLOG |
| O47-F06 | AGF's confidence gradient applied to content claims about external evidence but NOT to its own structural architectural decisions ("max delegation depth: 3", "outermost Ring 2 wins", trust-degradation asymmetry) | Medium | Established | Defensibility | **open** 2026-05-01 — editorial; requires `<Confidence level="informed">` markers applied to load-bearing structural choices in shared-vocabulary.md, agentic-primitives.md (Composability Interface signal restrictions, Governance Collapse Rule, trust ladder asymmetry, per-signal emission rules); pair with G55-F05 + G5-F18 editorial pass |
| O47-F07 | Trust-Ladder demotion against mandatory gate is "rejected at policy layer before emission" — no audit artifact for the rejected attempt; sentinel adversaries can probe gate mandatoriness | Low | Informed | Mechanical Integrity | **open** 2026-05-01 — structural; requires distinct observability event (`trust_demotion_rejected` with rejection reason + blocking policy) at policy layer, no GDR; queued in BACKLOG |
| X-M04 | (Cross-model meta) Runtime Contract Gap — convergent on G55-F01/F02/F03/F04 + O47-F02/F03; AGF specifies architectural objects well, distributed-systems failure semantics weakly. Co-equal structural blocker with F33. | High | Established | Actionability + Coherence | **open** 2026-05-01 — structural; requires new canonical doc "Runtime Integration Contract" covering GDR/event atomicity, signal arbitration, DELEGATE handshake, ESCALATE/DEFER lifecycle, idempotency, partial-failure semantics. Promotes from atomic findings to meta-tier per cross-model agreement. |
| X-M05 | (Cross-model meta) Standards-Fidelity Gap — Gemini Critical + Informed findings on ISO 42001 / EU AI Act mappings reveal that crosswalks are LLM-fuzzy synthesis rather than verified semantic alignment; Annex SL guess for ISO 42001 Annex A controls is hazardous; AGF's keyword-matched ("logs", "records") synthesis lapses where regulations require precision. | High | Established | Defensibility + Credibility | **open** 2026-05-01 — partially resolved (GEM-F01/F02 fixes shipped this session); residual scope: independent semantic audit of ALL relationship-to-frameworks.md crosswalks + Annex A verification + EU AI Act Articles 9/10/15/16/20/26/62 (cited but not deeply audited). New structural blocker class. |
| X-M06 | (Cross-model meta) Confidence-Gradient Stops at Architecture Boundary — Opus F06 + GPT-5.5 F05 convergent: AGF dogfoods the gradient on content claims about external evidence but leaves its own structural architectural decisions unmarked (presented as flat declaratives). The architecture appears more settled than the framework itself believes. | Medium | Established | Defensibility + Credibility | **open** 2026-05-01 — editorial; requires confidence-marker pass on shared-vocabulary.md, agentic-primitives.md structural claims, plus rewrite of confidence-levels.mdx coverage scope ("load-bearing empirical, comparative, novel, AND structural claims"). Single coherent editorial pass closes both atomic findings + this meta. |
...
| 2026-05-01 | Gemini 3.1 Pro (standards lens) | 7 | 8 | **5** | 7 | 6 | 6 | 6 | 8 | **6.6** |
| 2026-05-01 | GPT-5 Codex (adversarial practitioner lens) | 7 | 7 | 6 | 7 | 6 | 6 | **6** | 7 | **6.5** |
| 2026-05-01 | Claude Opus 4.7 (architectural coherence lens) | 7 | 6 | 7 | 7 | 6 | 7 | 7 | 7 | **6.75** |
| **2026-05-01** | **3-model multi-lens average** | **7** | **7** | **6** | **7** | **6** | **6.3** | **6.3** | **7.3** | **~6.6** |
