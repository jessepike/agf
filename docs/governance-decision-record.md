...
- **Not the Risk Decision Graph (RDG).** The RDG (see `docs/decision-intelligence.md`) is AGF's rich internal reasoning substrate — claims, beliefs, evidence relationships, scenarios, decision options. GDRs are the externally shareable serialization of the RDG's `Final Decision` and `Approval` nodes plus their supporting context. The RDG holds the full graph; GDR is the export format for governance handoffs.
...
Per `docs/profiles/observability-profile.md` § Known Limitations and Operational Realities, governance records MUST minimize sensitive content. GDRs follow the same principle and extend it as a normative requirement:
...
|---|---|---|
| Gate resolution records, evidence packages, human decision records, override logs (Primitive #8) | `docs/profiles/grc-profile.md` Evidence Artifacts table | GDR IS the format for the resolution / human-decision / override records. **Gate trigger records** (the event that a gate was fired) are observability events per Primitive #10 (`gate_triggered`), NOT GDRs. The `gate_triggered` event creates the corresponding `pending` GDR; on resolution, the `gate_resolved` event references the GDR by `decision_id`. |
| Trust level history, promotion/demotion records, calibration justifications (Primitive #11) | `docs/profiles/grc-profile.md` Evidence Artifacts table | GDR IS the format for promotion/demotion records. |
| `gate_triggered`, `gate_resolved` observability events (Primitive #10) | `docs/profiles/observability-profile.md` Governance events | Observability events log THAT a decision occurred (and its enum value). GDRs RECORD the decision with full reasoning. Both emitted at gate resolution; not redundant. The event references the GDR by `decision_id`. |
| `Final Decision` and `Approval` nodes in the Risk Decision Graph (RDG) | `docs/decision-intelligence.md` Risk Decision Graph | The RDG is AGF's rich internal reasoning substrate (claims, beliefs, evidence relationships, scenarios). The GDR is the externally shareable serialization of the Final Decision + Approval nodes plus their supporting context. RDG holds the full graph; GDR is the export format for governance handoffs. |
| Verification decision records (pass/revise/fail per output) for Primitive #1 | `docs/profiles/grc-profile.md` Evidence Artifacts table | NOT a GDR. Verification records are produced inside Ring 1 by the Verifier as part of normal output verification — they are observability event records, not gate-boundary decisions. Distinct artifact class. |
...
GDRs provide evidence in support of regulatory record-keeping and decision-documentation requirements. They do not by themselves satisfy these requirements; the host organization's overall governance program does. Specific regulatory mappings:

- **EU AI Act Art. 14 (Human Oversight)** — GDRs capture the authorizer identity, evidence presented, and decision rationale for gates that satisfy Art. 14 oversight requirements. This is GDR's primary EU AI Act mapping.
- **EU AI Act Art. 12 (Record-keeping)** — Art. 12 mandates **automatic event recording throughout the high-risk system's lifecycle** (continuous telemetry, not governance decision records). The primary AGF mapping for Art. 12 is therefore **Agentic Observability** events (Primitive #10), not GDRs. GDRs *complement* the Art. 12 event log by serializing the governance-decision context at gate boundaries; the host organization satisfies Art. 12 through observability-event retention and integrity controls. See `docs/profiles/observability-profile.md` for the canonical Art. 12 evidence story.
...
- **ISO/IEC 42001** — GDRs support the **documented information** requirements (Clause 7.5) and the **operational planning and control** requirements (Clause 8.1) where governance decisions are operational outputs that need to be evidenced. GDRs do **not** map to Clause 9.3 (Management Review) — that clause requires top management to review the entire AI Management System (AIMS) at planned intervals; a runtime governance decision record is an operational output, not a systemic management-review mechanism. <Confidence level="informed">ISO 42001 clause numbers used here are mapped from Annex SL where applicable. Annex A controls (which are AI-specific and not in Annex SL) are NOT yet independently verified against the published ISO 42001 text; the broader Annex A crosswalk is queued in `BACKLOG.md` as a pre-v0.3 standards-fidelity audit item.</Confidence>
- **AICM** — where AICM controls require documented governance decisions, GDRs provide a consistent format. Specific control crosswalks live in `docs/profiles/grc-profile.md`.
...
A schema validator and emission helpers will be published as part of the AGF tooling release. The first reference implementation is the AGF Tool Gate MVP (forthcoming).
