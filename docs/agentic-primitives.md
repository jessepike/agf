...
- **Self-improvement costs:** Reflexion-style learning loops (Ring 3 patterns) are estimated to consume significantly more tokens than single executions — multi-trial iterative learning requires repeated execution, evaluation, and refinement cycles. The exact multiplier varies by task and implementation (Shinn et al., NeurIPS 2023, describe multi-trial architectures but do not publish a single token multiplier). Ring 3 learning is a genuine investment, not a rounding error. This validates our framing of Ring 3 as an investment that must demonstrate ROI through efficiency gains in Rings 0-2.
...
- **Memory routing at scale:** When dozens of agents share memory/KB, how do you prevent noise from drowning signal? *Partial answer (Mar 2026): a namespaced memory pattern (global, project-scoped, session-scoped) provides a first-order filter. But namespace alone isn't enough — topic relevance scoring at query time is the real mechanism. Vector-search-backed knowledge bases do this natively; pure key-value memory stores generally do not yet.*
...
*This is the AGF foundational layer document. For the framework overview, see [AGF: A Reference Architecture for Governed Agentic Systems](agf-reference-architecture.md). For audience-specific views, see the domain profiles: [Security](../profiles/security-profile.md) · [Platform](../profiles/platform-profile.md) · [GRC](../profiles/grc-profile.md) · [AI Engineering](../profiles/ai-engineering-profile.md) · [Observability](../profiles/observability-profile.md). For capability layers built on these primitives, see [Decision Intelligence](decision-intelligence.md) and [Agentic Observability](agentic-observability.md).*
