...
AGF ships human-readable primitive forms today. Machine-readable primitive schemas (dual-form artifacts), FAIR-CAM control-effectiveness templates, and full maturity-level conformance criteria are planned for v0.2. Where this document describes AGF "operationalizing at machine speed," treat that as the architectural commitment — the artifacts that make it real are the next major work product.
