...
- [x] **External review** — Three independent reviews (Claude, GPT, Gemini). SIEM analogy qualified (partially holds, broader framing added). Correlation rules expanded from 18→30 with OWASP ASI + MITRE ATLAS tags, added: goal hijacking, tool misuse, supply chain, cascading failure, data exfil, resource exhaustion, confused deputy, missing telemetry, data governance violation. OTel signal mapping added (spans vs span events vs log records vs metrics). Market context updated (Microsoft Agent 365, AWS AgentCore, Zenity, Gartner $492M). Reviews archived. DONE.
