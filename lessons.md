...
- [pending] **Ground primers in canonical wording before drafting.** Reading `intent.md` + `DECISIONS.md` before writing the primer caught wording that would have contradicted canonical sources (e.g., dual-form is load-bearing AT GATE BOUNDARIES per D5, not all ring signals — easy to mis-state if drafting from memory). Discipline: always read the sacred + decision sources before writing any synthesis that quotes from them.

## 2026-04-25

- [pending] **Catch-22 dissolution requires completing the underlying work, not editorial polish.** F32 added a "audit not yet uniform" caveat to fix an overclaim; pass 4 then flagged the caveat itself as transition-state leakage. Removing it would restore the original overclaim. Path A resolved cleanly only by actually executing the underlying audit + diagram regen — then the caveat could be deleted honestly. Pattern: when honest disclosure scores worse than no claim, the only fix is to make the claim true.
- [pending] **Verification passes still find new editorial defects even when the targeted finding closes.** Codex pass 5 confirmed F32 dissolved, returned same 7/10 score floor, AND surfaced a stale-Roadmap regression (Path A had shipped but `/roadmap` "Now (in flight)" still listed it as pending). Lesson: any commit that ships a Roadmap-tracked item must update the Roadmap in the same commit, or a follow-on reviewer will flag the staleness. "Ship the work then update the roadmap" sequencing matters.
